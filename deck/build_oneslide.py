...
def frame(style, x, y, w, h, body):
    return (f'<draw:frame draw:style-name="{style}" svg:x="{x}cm" svg:y="{y}cm" '
...
def build():
    ...
    img = frame("frImg", MARGIN, 0.95, disp_w, disp_h,
                f"<draw:image><office:binary-data>{wm}</office:binary-data></draw:image>")
    head_txt = frame("frPlain", 13.4, 1.15, 19.0, 2.8, "<draw:text-box>"
                     + p("Tagline", "Watchful shadows. Passive defense.")
                     + p("Lede", "Passive counter-drone sensor mesh for Belgium: detect, locate, warn -- before the threat arrives.")
                     + "</draw:text-box>")
    rule = (f'<draw:rect draw:style-name="Rule" svg:x="{MARGIN}cm" svg:y="4.1cm" '
    ...
    footer = frame("frPlain", MARGIN, 18.35, PW - 2*MARGIN, 0.7, "<draw:text-box>"
                   + p("Footer", "TENEBRIS  |  1-minute overview  |  Detect. Locate. Warn. Save lives with cheap, silent technology.")
                   + "</draw:text-box>")
    ...
    a += (f'<style:style style:name="dpBg" style:family="drawing-page">'
          f'<style:drawing-page-properties draw:fill="solid" draw:fill-color="{NIGHT}"/></style:style>')
    a += ('<style:style style:name="frPlain" style:family="graphic">'
          '<style:graphic-properties draw:fill="none" draw:stroke="none" '
          'draw:textarea-vertical-align="middle" fo:padding="0cm"/></style:style>')
    a += ('<style:style style:name="frImg" style:family="graphic">'
          '<style:graphic-properties draw:fill="none" draw:stroke="none" '
          'draw:textarea-vertical-align="middle"/></style:style>')
    a += (f'<style:style style:name="Card" style:family="graphic">'
    ...
    a += (f'<style:style style:name="Rule" style:family="graphic">'
    ...
    a += (f'<style:page-layout style:name="PL"><style:page-layout-properties '
          f'fo:page-width="{PW}cm" fo:page-height="{PH}cm" '
          f'style:print-orientation="landscape"/></style:page-layout>')
    a += "</office:automatic-styles>"
    master = ('<office:master-styles><style:master-page style:name="Tenebris" '
    ...
    page = (f'<draw:page draw:name="overview" draw:style-name="dpBg" '
