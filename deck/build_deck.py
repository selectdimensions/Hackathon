...
SLIDES = [
    {"kind": "title"},
    {"kind": "content", "kicker": "The problem",
    ...
    {"kind": "content", "kicker": "How it works",
    ...
    {"kind": "content", "kicker": "Built to a hard spec",
    ...
    {"kind": "content", "kicker": "Modular by design",
    ...
    {"kind": "content", "kicker": "Deploy & resilience",
    ...
    {"kind": "table", "kicker": "Head to head",
    ...
    {"kind": "content", "kicker": "Scale to Belgium",
    ...
    {"kind": "section", "kicker": "Live demo · 3 minutes",
...
def bullet_list(items, style="Bullet"):
    lis = "".join(
        f'<text:list-item>{p(style, t)}</text:list-item>' for t in items
    )
...
def title_block(kicker, title):
    ...
    # accent rule
    out += (
        f'<draw:rect draw:style-name="Rule" svg:x="1.95cm" svg:y="4.35cm" '
        f'svg:width="6.2cm" svg:height="0.09cm"><text:p/></draw:rect>'
    )
    return out
...
def footer(page_no):
    out = ""
    out += frame("frPlain", 1.9, 18.05, 22.0, 0.7,
                 textbox(p("Footer", "TENEBRIS  ·  Watchful shadows. Passive defense.")))
    out += frame("frPlainR", 27.0, 18.05, 4.9, 0.7,
...
def slide_title(idx, s, wm_b64, wm_w, wm_h):
    ...
    body += img
    ty = 4.2 + disp_h + 0.6
    body += frame("frPlainC", 2.0, ty, PW - 4.0, 1.2,
                  textbox(p("Tagline", "Watchful shadows. Passive defense.")))
    body += frame("frPlainC", 2.0, ty + 1.3, PW - 4.0, 1.6, textbox(p(
        "Lede",
        "A distributed passive sensor mesh that detects, locates, and warns — before the threat arrives.")))
    body += frame("frPlainC", 2.0, PH - 1.5, PW - 4.0, 0.8, textbox(p(
        "Footer", "8-minute brief + live demo  ·  Counter-drone defense for Belgium")))
...
def slide_close(idx):
    ...
    body += frame("frPlainC", 2.0, 9.8, PW - 4.0, 1.2,
                  textbox(p("Tagline", "Watchful shadows. Passive defense.")))
    body += frame("frPlainC", 2.0, 11.2, PW - 4.0, 1.0, textbox(p(
        "Lede", "Detect. Locate. Warn. Save lives — with cheap, silent technology.")))
    return draw_page(idx, body)


def slide_content(idx, s):
    body = title_block(s["kicker"], s["title"])
    body += frame("frPlain", 1.9, 5.1, 30.2, 12.4,
                  textbox(bullet_list(s["bullets"])))
    body += footer(idx + 1)
    return draw_page(idx, body)
...
def slide_section(idx, s):
    body = title_block(s["kicker"], s["title"])
    body += frame("frPlain", 1.9, 5.1, 30.2, 11.0,
...
def slide_table(idx, s):
    ...
    trs = ""
    # header row
    trs += "<table:table-row>" + "".join(
        cell(head[c], "cellHead", "ThAccent" if c == cols - 1 else "Th") for c in range(cols)
    ) + "</table:table-row>"
    ...
    table = (
        f'<table:table table:style-name="tbl">{colspec}{trs}</table:table>'
    )
...
# ----------------------------------------------------------------------------
def styles_xml():
    def para(name, font, size, color, bold=False, align="start", spacing=0.15, upper=False):
        b = ' fo:font-weight="bold"' if bold else ""
        tt = ' fo:text-transform="uppercase"' if upper else ""
        ls = f' fo:letter-spacing="0.05cm"' if upper else ""
        return (
    ...
    for nm, valign in [("frPlain", "top"), ("frPlainC", "middle"),
                       ("frPlainR", "top"), ("frImg", "middle")]:
        a += (
